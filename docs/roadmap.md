...
- [ ] 本地资产目录 `~/.muse`
- [ ] 内置 Persona ×2–3、Skill ×2–3、Tools（read/list/shell）
...
#### 阶段 1：CLI Runtime 内核（~2 周） — [计划](./v0.1/phase-1.md)

- `MuseHarness` 封装 AgentHarness
- `~/.muse` 初始化
- 内置 tools：`read_file`、`list_dir`、`run_command`
- JSONL Session 存储
- `muse start`：HTTP + SSE + CORS
- 调试：`muse chat` 或 curl 发 prompt

**里程碑**：终端/curl 能 SSE 收流；重启后会话可恢复。
...
**里程碑**：Web 配 Key，CLI 无 Key 可调模型；设备显示 online/offline。

---

#### 阶段 4：Web 聊天 MVP（~2–3 周） — [计划](./v0.1/phase-4.md)

- 登录、设备选择（读 Backend endpoint，直连 CLI SSE）
- Agent 选择与简易组装 UI
- 聊天页：流式 Markdown、thinking、tool call
- steer / followUp
...
#### 阶段 5：打磨与自用（~1 周） — [计划](./v0.1/phase-5.md)
