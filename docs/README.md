...
| [路线规划](./roadmap.md)                | 版本路线；v0.1 已交付，v0.2 进行中                 |
| **[开发指南](./development-guide.md)**  | Monorepo 联调、排错、市场验收、远程 CLI            |
| **[v0.1 交付说明](./releases/v0.1.md)** | v0.1 产品线终态（含 v0.1.1 增量）                  |
| **[v0.2 市场 v1](./v0.2/market.md)**    | 市场规格、实现步骤与验收标准                       |
| [当前阶段](./current-phase.md)          | 当前版本快照                                       |
| [API 协议](./protocols.md)              | REST 与 SSE 契约（含市场 API）                     |
...
Web ──HTTP──────► Backend      登录、设置、设备列表、市场目录
```
...
│   ├── cli/           # @museai/cli，本地命令 muse
│   ├── core/          # Agent 组装、资产加载（封装 pi AgentHarness）
│   ├── shared/        # API 类型、事件协议、i18n
│   ├── server/        # @museai/server，含 docker-compose.yml
│   └── web/           # 前端
└── docs/              # 本文档目录
```
