              ...
              <div key={index} className="flex flex-col gap-2 rounded-lg border border-border/70 p-3">
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    onChange={event =>
                      onChange({
                        ...value,
                        extraModels: extraModels.map((item, itemIndex) => (itemIndex === index ? { ...item, id: event.target.value } : item)),
                      })
                    }
                  />
                  <Input
                    ...
                    onChange={event =>
                      onChange({
                        ...value,
                        extraModels: extraModels.map((item, itemIndex) => (itemIndex === index ? { ...item, name: event.target.value } : item)),
                      })
                    }
                  />
                  <IconButton
                ...
                <HeadersEditor
                  ...
                  onChange={headers =>
                    onChange({
                      ...value,
                      extraModels: extraModels.map((item, itemIndex) => (itemIndex === index ? { ...item, headers } : item)),
                    })
                  }
                />
              </div>
