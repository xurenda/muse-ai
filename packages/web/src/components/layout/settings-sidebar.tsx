  ...
  return (
    <aside className="flex h-full min-h-0 flex-col">
      ...
      <nav className="flex flex-col px-2 pb-3">
        <Link to="/chat" className="ui-menu-item rounded-lg text-sidebar-foreground hover:bg-sidebar-accent/60">
          <ChevronLeft className="size-3.5 shrink-0 opacity-80" strokeWidth={2} />
          <span>{t('back')}</span>
        </Link>
        <div className="mt-4 flex flex-col gap-0.5">
          {settingsNavItems.map(item => (
            <SettingsNavLink key={item.to} to={item.to} icon={item.icon} label={t(item.labelKey)} end={'end' in item ? item.end : undefined} />
          ))}
        </div>
      </nav>
    </aside>
