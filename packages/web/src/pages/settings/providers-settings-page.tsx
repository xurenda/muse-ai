...
import {
  emptyProviderAdvanced,
  ProviderAdvancedForm,
} from '@/components/settings/provider-advanced-form'
...
function getApiKeyPlaceholder(
  provider: { authStatus: ProviderAuthStatus; apiKeyMask?: string },
...
function SettingsListExpandToggle({
  expanded,
  hiddenCount,
  showMoreLabel,
  showLessLabel,
  onToggle,
}: SettingsListExpandToggleProps) {
  if (hiddenCount <= 0) {
    return null
  }
...
export function ProvidersSettingsPage() {
  ...
  const loadConfig = useCallback(async () => {
    setLoading(true)
    try {
      const next = await fetchProvidersConfig()
      setConfig(next)
    } catch (loadError) {
  ...
  const handleSaveProvider = async (providerId: string) => {
    ...
    try {
      ...
      await saveProviderAdvancedConfig(providerId, {
  ...
  const handleSaveCustom = async () => {
    ...
    try {
      await saveCustomProvider(providerId, {
  ...
  const sortedApiKeyProviders = useMemo(
    () => sortApiKeyProviders(config?.apiKeyProviders ?? []),
    [config?.apiKeyProviders],
  )
  ...
  const showAllApiKeyProviders =
    ...
    (expandedProviderId !== null &&
      sortedApiKeyProviders.findIndex((provider) => provider.id === expandedProviderId) >=
        DEFAULT_VISIBLE_PROVIDER_COUNT)
  const showAllCustomProviders =
    customListExpanded ||
    (expandedCustomId !== null &&
      expandedCustomId !== NEW_CUSTOM_PROVIDER_ID &&
      customProviders.findIndex((provider) => provider.id === expandedCustomId) >=
        DEFAULT_VISIBLE_PROVIDER_COUNT)
  const visibleApiKeyProviders = showAllApiKeyProviders
    ? sortedApiKeyProviders
    : sortedApiKeyProviders.slice(0, DEFAULT_VISIBLE_PROVIDER_COUNT)
  const visibleCustomProviders = showAllCustomProviders
    ? customProviders
    : customProviders.slice(0, DEFAULT_VISIBLE_PROVIDER_COUNT)
  ...
  const renderCustomProviderForm = (isNew: boolean) => (
    <div className="flex flex-col gap-4" onClick={(event) => event.stopPropagation()}>
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            disabled={!isNew && busyKey === `custom-delete:${expandedCustomId}`}
      ...
      <SettingsFieldRow label={t('providers.custom.apiKeyOptional')}>
        <Input
          type="password"
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-8">
        ...
        >
          ...
          {visibleApiKeyProviders.map((provider) => {
            const readOnly = provider.authStatus === 'env'
            const isExpanded = expandedProviderId === provider.id
            const isConfigured = provider.authStatus !== 'missing'
            const statusLabel = isConfigured
              ? t('providers.authStatus.configured')
              : t('providers.authStatus.missing')
            ...
            return (
              <SettingsRow
                ...
                title={provider.name}
                onClick={
                  readOnly ? undefined : () => toggleProvider(provider.id, readOnly, provider.advanced)
                }
                expanded={
                  isExpanded ? (
                    <div className="flex flex-col gap-4" onClick={(event) => event.stopPropagation()}>
                      ...
                      <ProviderAdvancedForm
                        ...
                        onChange={(next) =>
                          setAdvancedDrafts((current) => ({ ...current, [provider.id]: next }))
                        }
                        actions={
                          <>
                            <Button
        ...
        >
          ...
          {expandedCustomId === NEW_CUSTOM_PROVIDER_ID ? (
            <div className="px-4 py-3.5">{renderCustomProviderForm(true)}</div>
          ) : null}
        </SettingsSection>
